...
function GAFE_TrialsSchedule:Initialize(control)
    ...
    self.scroll = self.control:GetNamedChild("Scroll")
...
function GAFE_TrialsSchedule:InitializeFragment()
    ...
    self.scrollList:SetAnchor(TOPLEFT, self.scrollChild, TOPLEFT, 0, 0)
    self.scrollList:SetAnchor(BOTTOMRIGHT, self.scrollChild, BOTTOMRIGHT, 50, 0)

    -- Add data to scroll list.
    local dataItems = {
    }
    for i=1,100 do
        dataItems[i] = {name= i..'asdf asdf asdf asdfasdfasdfa sdfa sdfa sdfa  '}
    end
